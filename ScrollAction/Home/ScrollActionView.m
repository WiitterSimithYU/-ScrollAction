...
- (instancetype)initWithFrame:(CGRect)frame WithImageName:(NSString *)iconImageName WithTitle:(NSString *)title{
    ...
    if (self) {
        UIImageView *iconImageview = [[UIImageView alloc]initWithFrame:CGRectMake((self.frame.size.width-(self.frame.size.height-15)*0.7)*0.5, 5,(self.frame.size.height-15)*0.7,(self.frame.size.height-15)*0.7)];
        iconImageview.image = [UIImage imageNamed:iconImageName];
        [self addSubview:iconImageview];
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(5,CGRectGetMaxY(iconImageview.frame) + 5, self.frame.size.width-10, (self.frame.size.height-15)*0.3)];
        label.text = title;
...
- (void)createShareAcitonViewWithFrame:(CGRect)frame WithSourceArray:(NSArray *)arr WithIconArray:(NSArray *)iconArray{
    ...
    _scrollerView = [[UIScrollView alloc]initWithFrame:CGRectMake(0,0, self.frame.size.width, self.frame.size.height-20)];
    ...
    _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake((self.frame.size.width - 100)/2.0, self.frame.size.height - 20, 100, 20)];
    ...
    [self addSubview:_pageControl];
     ...
     for (int p = 0; p < page; p++) {
         ...
         multView.backgroundColor = MAIN_RGBColor(242, 245, 247,1);
         ...
         for (int i = p*NUMBER; i < arr.count; i++) {
             ...
             actionView.tag = i;
             actionView.backgroundColor = MAIN_RGBColor(242, 245, 247,1);
